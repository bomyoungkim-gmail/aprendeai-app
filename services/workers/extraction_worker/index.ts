...
async function processExtraction(payload: ExtractionMessage) {
  ...
  try {
    ...
    await prisma.contentExtraction.update({
      where: { contentId },
...
async function extractFromPDF(file: any): Promise<{ text: string; metadata: any }> {
...
async function extractFromDOCX(file: any): Promise<{ text: string; metadata: any }> {
  const mockFilePath = path.join(
    process.env.STORAGE_LOCAL_PATH || './uploads',
...
async function extractFromImage(file: any): Promise<{ text: string; metadata: any }> {
