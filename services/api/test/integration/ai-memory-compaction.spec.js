...
describe("Memory Compaction", () => {
    ...
    it("should verify memory.compact queue exists", async () => {
        ...
        console.log(`✅ Queue 'memory.compact' exists with ${queueInfo.messageCount} messages`);
    });
    it("should simulate memory job enqueueing", async () => {
        ...
        const sent = rabbitChannel.sendToQueue("memory.compact", Buffer.from(JSON.stringify(testJob)), { persistent: true });
    ...
    it("should verify compact state storage format", async () => {
        ...
        await redis.set(stateKey, JSON.stringify(compactState), "EX", 180 * 24 * 3600);
        ...
        console.log(`✅ Compact state validated (${sizeBytes} bytes, TTL: 180 days)`);
    ...
    it("should test memory deduplication logic", async () => {
        ...
        expect(isNew1).toBe(1);
        await redis.expire(dedupKey, 365 * 24 * 3600);
        const isNew2 = await redis.setnx(dedupKey, "1");
        expect(isNew2).toBe(0);
        console.log("✅ Deduplication logic validated");
        await redis.del(dedupKey);
    });
});
//# sourceMappingURL=ai-memory-compaction.spec.js.map