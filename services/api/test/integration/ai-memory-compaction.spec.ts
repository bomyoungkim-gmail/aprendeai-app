...
describe('Memory Compaction', () => {
  ...
  it('should verify compact state storage format', async () => {
    ...
    // Store compact state
    await redis.set(stateKey, JSON.stringify(compactState), 'EX', 180 * 24 * 3600); // 180 days
  ...
  it('should test memory deduplication logic', async () => {
    ...
    const hash = crypto.createHash('sha256').update(memoryText.toLowerCase().trim()).digest('hex').substring(0, 16);
