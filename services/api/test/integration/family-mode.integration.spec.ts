...
describe('Family Mode Integration Tests (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    authToken = authHelper.generateAuthHeader({ ...educatorData, id: educator.id });
  ...
  afterAll(async () => {
    // Cleanup
    if (readingSessionId) await prisma.readingSession.deleteMany({ where: { id: readingSessionId } });
    if (teachBackReadingSessionId) await prisma.readingSession.deleteMany({ where: { id: teachBackReadingSessionId } });
