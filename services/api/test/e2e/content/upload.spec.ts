...
test.describe("Content Upload", () => {
  ...
  test("user can upload PDF file", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/Upload.*Conteúdo|Upload Content/i"),
    ).toBeVisible();
    ...
    await expect(page.locator("text=/Upload.*Conteúdo/i")).toBeHidden({
      timeout: 15000,
    });
  ...
  test("upload validates file type - rejects invalid files", async ({
    ...
    await expect(
      page.locator("text=/não suportado|not supported|invalid/i"),
