...
describe('Family + Classroom E2E Tests', () => {
  ...
  beforeAll(async () => {
    ...
    userId = parent.id;
    authToken = authHelper.generateAuthHeader({ id: parent.id, email: parent.email, name: parent.name });
  ...
  describe('E2E: Classroom Teacher Dashboard', () => {
    it('should display classroom dashboard with multiple students', async () => {
      ...
      const student2 = await prisma.user.create({ data: { email: `s2_${Date.now()}@test.com`, name: 'S2', role: 'COMMON_USER', schoolingLevel: 'K12_LOWER' }});
      await request(app.getHttpServer())
        .post(`/api/v1/classrooms/${classroomId}/enroll`)
        .set('Authorization', authToken)
  ...
  describe('E2E: 1:1 Intervention Trigger', () => {
    it('should trigger intervention on help request', async () => {
      ...
      const interventionLearner = await prisma.user.create({
        data: { email: `int_learn_${Date.now()}@test.com`, name: 'Struggling Student', role: 'COMMON_USER', schoolingLevel: 'K12_LOWER' }
      });
