...
test.describe('Real-Time Collaboration - WebSocket', () => {
  ...
  test('session start event propagates to all members immediately', async ({ browser }) => {
    ...
    try {
      ...
      await expect(memberPage.locator('text=/RUNNING|Em andamento/i')).toBeVisible();
  ...
  test('vote submission updates vote count in real-time', async ({ browser }) => {
    ...
    try {
      ...
      await expect(voteCountLocator).not.toHaveText(initialText || '', { timeout: 2000 });
  ...
  test('chat messages appear in real-time', async ({ browser }) => {
    ...
    try {
      ...
      const messagePromise = page2.waitForSelector(`text=${testMessage}`, { timeout: 3000 });
  ...
  test('connection status shows Live when connected', async ({ page }) => {
    ...
    await expect(page.locator('text=/Live|Conectado/i, [data-ws-status="connected"]')).toBeVisible({ timeout: 5000 });
  });
  ...
  test.skip('users in different sessions dont see each others events', async ({ browser }) => {
