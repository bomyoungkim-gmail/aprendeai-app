...
import { loginAs, createGroup, waitForWebSocketConnection } from '../helpers/test-helpers';
...
test.describe('Study Groups - Session Flow', () => {
  ...
  test('session shows correct initial status CREATED', async ({ page }) => {
    ...
    // Status badge
    await expect(page.locator('[data-testid="session-status"], text=/CREATED|Criada/i')).toBeVisible();
    
    // Start button should be visible for facilitator
    await expect(page.locator('button:has-text("Start"), button:has-text("Iniciar")')).toBeVisible();
  });
  ...
  test('members can see assigned roles', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="user-role"], text=/FACILITATOR|TIMEKEEPER|CLARIFIER|CONNECTOR|SCRIBE/i')).toBeVisible();
  });

  test('facilitator starts session and status changes to RUNNING', async ({ page }) => {
    await page.goto(`/groups/test-group/sessions/test-session`);
    ...
    await expect(page.locator('text=/RUNNING|Em andamento/i')).toBeVisible({ timeout: 5000 });
  ...
  test('users can submit votes', async ({ page }) => {
    ...
    await expect(page.locator('text=/Vote submitted|Voto enviado/i')).toBeVisible();
  });
  ...
  test('cannot advance without all votes (409 error)', async ({ page }) => {
    ...
    await expect(page.locator('text=/waiting for|aguardando|all members/i')).toBeVisible();
  });

  test('facilitator advances to DISCUSSING after all votes', async ({ page }) => {
    ...
    // Phase should change
    await expect(page.locator('text=/DISCUSSING|Discussão/i')).toBeVisible({ timeout: 3000 });
  });
  ...
  test('chat is visible during DISCUSSING phase', async ({ page }) => {
    ...
    // Chat panel
    await expect(page.locator('[data-testid="chat-panel"], text=/Chat.*Discussão/i')).toBeVisible();
    
    // Message input
    await expect(page.locator('textarea[placeholder*="mensagem"]')).toBeVisible();
  });
  ...
  test('scribe can submit group explanation', async ({ page, context }) => {
    ...
    // Success
    await expect(page.locator('text=/Explanation submitted|Explicação enviada/i')).toBeVisible();
  });
  ...
  test('shared card appears after submission', async ({ page }) => {
    ...
    await expect(page.locator('text=This is the group explanation')).toBeVisible();
  });
  ...
  test('facilitator ends session and shows final results', async ({ page }) => {
    ...
    await expect(page.locator('text=/FINISHED|Finalizada/i')).toBeVisible({ timeout: 5000 });
