...
export default async () => {
  ...
  process.env.DATABASE_URL = process.env.DATABASE_URL || 'postgresql://postgres:postgres@127.0.0.1:5432/aprendeai_test';
  process.env.RABBITMQ_URL = process.env.RABBITMQ_URL || 'amqp://guest:guest@localhost:5672';
  process.env.REDIS_URL = process.env.REDIS_URL || 'redis://localhost:6379';

  // AWS/Email (mock values for tests)
  process.env.AWS_REGION = 'us-east-1';
  process.env.AWS_ACCESS_KEY_ID = 'test';
  process.env.AWS_SECRET_ACCESS_KEY = 'test';
  process.env.SES_FROM_ADDRESS = 'test@example.com';

  // AI Service (mock for tests)
