...
export class AssetsService {
  ...
  async generateAsset(
    ...
    const ent = await this.entitlements.resolve('USER', userId, 'STAGING'); // TODO: Use correct environment
    
    if (!ent.features.ai_cornell_assist_enabled) {
      throw new ForbiddenException('AI Cornell Assist is not enabled for this user');
    }
  ...
  private calculateCacheHash(contentId: string, dto: GenerateAssetDto): string {
    const highlightsHash = dto.selectedHighlightIds
      ? crypto.createHash('sha256').update(dto.selectedHighlightIds.sort().join(',')).digest('hex')
      : 'none';
