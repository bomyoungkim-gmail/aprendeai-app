...
import { EducationLevel, SourceType } from '../dto/question-bank.dto';
...
describe('QuestionSelectionService', () => {
  ...
  beforeEach(async () => {
    ...
    aiGenerator = module.get<AIQuestionGeneratorService>(AIQuestionGeneratorService);
  });
  ...
  describe('getQuestionsForUser', () => {
    ...
    it('should return questions from DB if enough exist', async () => {
      ...
      (prisma.questionBank.findMany as jest.Mock).mockResolvedValue(mockQuestions);
    ...
    it('should generate questions via AI if DB has insufficient questions', async () => {
      ...
      (prisma.questionBank.findMany as jest.Mock).mockResolvedValue(dbQuestions);
      (mockAiGenerator.generate as jest.Mock).mockResolvedValue(aiQuestions);
      (prisma.questionBank.create as jest.Mock).mockImplementation((args) => ({ id: 'new', ...args.data }));

      const result = await service.getQuestionsForUser(params);

      expect(result).toHaveLength(5); // 1 DB + 4 AI
      expect(prisma.questionBank.findMany).toHaveBeenCalled();
      expect(mockAiGenerator.generate).toHaveBeenCalledWith(expect.objectContaining({ count: 4 }));
      expect(prisma.questionBank.create).toHaveBeenCalledTimes(4);
    });

    it('should return available questions if AI fails', async () => {
      const dbQuestions = [{ id: '1', question: {} }];
      (prisma.questionBank.findMany as jest.Mock).mockResolvedValue(dbQuestions);
      (mockAiGenerator.generate as jest.Mock).mockRejectedValue(new Error('AI Error'));
