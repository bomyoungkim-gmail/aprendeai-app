...
export class ShareContentUseCase {
  ...
  async execute(userId: string, contentId: string, dto: { contextType: ShareContextType, contextId: string, permission: SharePermission }): Promise<ContentShare> {
    // Permission Check (from original service)
    if (dto.contextType === ShareContextType.CLASSROOM && dto.permission === SharePermission.ASSIGN) {
      const canAssign = await this.permissions.canCreateClassroom(userId);
      if (!canAssign) {
        throw new ForbiddenException('Only verified teachers can assign content in classrooms');
      }
    }
