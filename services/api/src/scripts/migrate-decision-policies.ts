...
import {
  migrateLegacyPolicy,
  hasLegacyKeys,
} from "../policies/legacy-policy-migration";
...
async function migrateInstitutionPolicies(
  dryRun: boolean,
...
async function migrateFamilyPolicies(dryRun: boolean): Promise<MigrationStats> {
  ...
  for (const fam of families) {
    try {
      ...
      if (hasLegacyKeys(policy)) {
        console.log(
          `  Migrating family policy: ${fam.family_id} (learner: ${fam.learner_user_id})`,
        );
...
async function main() {
  ...
  try {
    ...
    if (totalErrors > 0) {
      ...
      [...instStats.errors, ...famStats.errors].forEach((err) =>
        console.error(`  - ${err}`),
      );
    }
