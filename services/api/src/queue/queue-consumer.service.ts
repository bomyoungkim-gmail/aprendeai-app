...
export class QueueConsumerService implements OnModuleInit {
    ...
    try {
      ...
      this.channel.consume(
        ...
        async (msg) => {
          ...
          try {
            const content = JSON.parse(msg.content.toString()) as ContentProcessMessage;
    ...
    try {
      ...
      await this.prisma.contentVersion.create({
        data: {
          contentId,
          targetLanguage: lang,
          schoolingLevelTarget: schoolingLevel,
      ...
      // Also emit success because we DID create a version
       this.notificationsGateway.emitContentUpdate(
        contentId,
        'simplification',
      );
    }
  ...
  private async handleAssessment(contentId: string, text: string, level?: string) {
