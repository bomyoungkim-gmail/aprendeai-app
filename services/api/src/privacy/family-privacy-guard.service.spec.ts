...
describe('FamilyPrivacyGuard', () => {
  ...
  const mockDashboardData: EducatorDashboardData = {
    ...
    alerts: [
      { type: 'SLUMP', severity: 'MED', message: 'Student struggling with vocab' },
    ],
  ...
  describe('canViewField', () => {
    ...
    it('should allow topBlockers for AGGREGATED_PLUS_TRIGGERS', () => {
      expect(
        service.canViewField('topBlockers', PrivacyMode.AGGREGATED_PLUS_TRIGGERS),
      ).toBe(true);
    });
    ...
    it('should deny detailedLogs for all modes', () => {
      ...
      expect(
        service.canViewField('detailedLogs', PrivacyMode.AGGREGATED_PLUS_TRIGGERS),
      ).toBe(false);
    });
  });
