...
describe('FeedbackGenerationService', () => {
  ...
  it('should return cached feedback if available (Cache Hit)', async () => {
    ...
    // Act
    const result = await service.generateFeedback(mockQuestion, mockUserAnswer, mockCorrectAnswer);
  ...
  it('should generate feedback via LLM if cache miss (Cache Miss)', async () => {
    ...
    expect(redisService.get).toHaveBeenCalled();
    expect(llmService.generateText).toHaveBeenCalledWith(expect.stringContaining(mockQuestion.questionText), expect.any(Object));
    expect(redisService.set).toHaveBeenCalledWith(expect.any(String), expectedFeedback, expect.any(Number));
  });
