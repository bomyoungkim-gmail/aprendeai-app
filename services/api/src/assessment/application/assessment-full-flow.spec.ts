...
const mockPrismaService = {
  ...
  assessment_attempts: { findMany: jest.fn(), update: jest.fn(), findUnique: jest.fn(), create: jest.fn() },
  assessment_questions: { findUnique: jest.fn() },
  assessments: { findFirst: jest.fn(), create: jest.fn(), findUnique: jest.fn() },
  ...
  usage_events: { aggregate: jest.fn().mockResolvedValue({ _sum: { quantity: 100 } }) },
...
const mockScaffoldingService = {
  ...
  getThresholdMultipliers: jest.fn().mockReturnValue({ doubtThreshold: 3, checkpointFailThreshold: 2 }),
};
...
describe('Assessment Engine Full Flow Integration', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: IDecisionLogRepository, useValue: mockDecisionLogRepository },
        { provide: ScaffoldingSignalDetectorService, useValue: { detectSignal: jest.fn() } },
        { provide: FlowStateDetectorService, useValue: { detectFlowState: jest.fn() } },
        { provide: DcsCalculatorService, useValue: { calculateDcs: jest.fn() } },
        { provide: DcsIntegrationHelper, useValue: { 
    ...
    generationService = module.get<AssessmentGenerationService>(AssessmentGenerationService);
    evaluationService = module.get<AssessmentEvaluationService>(AssessmentEvaluationService);
  ...
  describe('1. Highlight-Aware Quiz Generation', () => {
    it('should generate quiz using DOUBT/EVIDENCE highlights', async () => {
      ...
      mockPrismaService.highlights.findMany.mockResolvedValue([
        { type: 'DOUBT', anchor_json: { text: 'I am confused about photosynthesis' } },
        { type: 'EVIDENCE', anchor_json: { text: 'Mitochondria is the powerhouse' } },
      ]);
      ...
      const quiz = await generationService.generateQuiz('content1', 'user1', 1, 'DIDACTIC' as any);

      expect(mockPrismaService.highlights.findMany).toHaveBeenCalled();
      expect(mockAiService.generateQuiz).toHaveBeenCalledWith(expect.stringContaining('DOUBT'));
  ...
  describe('2. Assessment Evaluation', () => {
    it('should evaluate open-ended answer via LLM', async () => {
      ...
      mockPrismaService.assessment_attempts.findUnique.mockResolvedValue({ id: 'att1' });

      mockAiService.evaluateAnswer.mockResolvedValue({
        structuredOutput: { score: 0.9, feedback: 'Great job' }
      });

      const result = await evaluationService.evaluateAnswer('att1', 'q1', 'X is basically Y');
  ...
  describe('3. Decision Integration (Scaffolding Adjustment)', () => {
    it('should INCREASE scaffolding after 2 poor attempts', async () => {
      ...
      mockPrismaService.reading_sessions.findUnique.mockResolvedValue({ phase: 'DURING' });
