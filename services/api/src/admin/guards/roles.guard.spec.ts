...
describe('RolesGuard', () => {
  ...
  describe('canActivate', () => {
    ...
    it('should allow access when user has matching legacy role', () => {
    ...
    it('should allow access when user has ANY matching role (tri-check)', () => {
      ...
      const context = createMockContext({
        ...
        contextRole: 'TEACHER', // matches!
        role: 'STUDENT', // doesn't match
      });
    ...
    it('should handle user with only legacy role field', () => {
      jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue(['TEACHER']);
      
      const context = createMockContext({
        id: 'user-123',
        role: 'TEACHER',
        // no systemRole or contextRole
      });
