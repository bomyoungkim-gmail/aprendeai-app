...
export class ManageFeatureFlagsUseCase {
  ...
  async toggle(id: string, enabled: boolean, reason: string | undefined, actor: { userId: string; role: string }): Promise<FeatureFlag> {
    const existing = await this.flagsRepo.findById(id);
    if (!existing) throw new NotFoundException("Feature flag not found");
  ...
  async delete(id: string, reason: string, actor: { userId: string; role: string }): Promise<void> {
    const existing = await this.flagsRepo.findById(id);
  ...
  async list(filter?: { environment?: FeatureFlagEnvironment; enabled?: boolean }) {
