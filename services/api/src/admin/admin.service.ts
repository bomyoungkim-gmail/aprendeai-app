...
export class AdminService {
  constructor(
    ...
    @Inject(IFeatureFlagsRepository) private readonly flagsRepo: IFeatureFlagsRepository,
  ...
  async updateFeatureFlag(id: string, data: any, actorUserId: string, actorRole: string) {
    return this.manageFlagsUseCase.update(id, data, { userId: actorUserId, role: actorRole });
  }

  async toggleFeatureFlag(id: string, enabled: boolean, reason: string | undefined, actorUserId: string, actorRole: string) {
    return this.manageFlagsUseCase.toggle(id, enabled, reason, { userId: actorUserId, role: actorRole });
  }

  async deleteFeatureFlag(id: string, reason: string, actorUserId: string, actorRole: string) {
    await this.manageFlagsUseCase.delete(id, reason, { userId: actorUserId, role: actorRole });
    return { deleted: true };
  }

  async evaluateFeatureFlag(key: string, userId?: string, institutionId?: string) {
  ...
  async updateUserStatus(userId: string, status: string, reason: string, actorUserId: string, actorRole: string) {
    return this.userManagementUseCase.updateUserStatus(userId, status, reason, { userId: actorUserId, role: actorRole });
  }

  async createImpersonationToken(targetUserId: string, actorUserId: string, actorRole: string, reason: string, durationMinutes: number) {
  ...
  async updateUserRoles(userId: string, roles: any, reason: string, actorUserId: string, actorRole: string) {
    // Stub kept
    return { count: 0 };
  }
