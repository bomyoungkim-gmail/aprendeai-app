...
import { SessionMetricsDto, DailyEngagementDto } from './dto/analytics-response.dto';

@Injectable()
export class AnalyticsService {
  constructor(private prisma: PrismaService) {}

  async getSessionMetrics(contentId: string, userId: string): Promise<SessionMetricsDto> {
    ...
    const totalTimeMs = timeEvents
      ...
      .reduce((acc, curr) => acc + ((curr.data as any)?.totalDurationMs || 0), 0);
    ...
    const interactions = await this.prisma.telemetry_events.groupBy({
      ...
      where: {
        user_id: userId,
        content_id: contentId,
        event_type: { in: ['HIGHLIGHT_CREATED', 'NOTE_CREATED', 'SCROLL_DEPTH'] }
      },
      _count: true
    });

    const highlightsCount = interactions.find(i => i.event_type === 'HIGHLIGHT_CREATED')?._count || 0;
    const notesCount = interactions.find(i => i.event_type === 'NOTE_CREATED')?._count || 0;
    
    // 3. Max Scroll
    const scrollEvents = await this.prisma.telemetry_events.findMany({
      where: { user_id: userId, content_id: contentId, event_type: 'SCROLL_DEPTH' },
      select: { data: true }
    });
    const maxScroll = scrollEvents.reduce((max, e) => Math.max(max, (e.data as any)?.scrollPercent || 0), 0);

    // 4. Mode
    const modeEvents = await this.prisma.telemetry_events.findFirst({
        where: { user_id: userId, content_id: contentId, event_type: 'CHANGE_MODE' },
        orderBy: { created_at: 'desc' }
    });
