...
export class SessionTrackingService {
  ...
  async handleSessionStart(event: SessionStartedEvent) {
    ...
    return this.trackUseCase.startSession(
      user_id,
      activity_type,
      content_id,
      source_id,
  ...
  async handleSessionFinish(event: SessionFinishedEvent) {
    return this.trackUseCase.finishSession(event.sessionId, {
      ...
      netFocusMinutes:
        event.net_focus_minutes || (event as any).netFocusMinutes,
  ...
  async findActiveSession(user_id: string, activityType?: string) {
    return this.trackUseCase["repository"].findActiveSession(
      user_id,
      activityType,
    );
  }
  ...
  async autoCloseAbandonedSessions(thresholdMinutes: number = 30) {
    const abandoned =
      await this.trackUseCase["repository"].findAbandonedSessions(
        thresholdMinutes,
      );
