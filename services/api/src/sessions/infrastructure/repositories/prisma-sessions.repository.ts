...
export class PrismaSessionsRepository implements ISessionsRepository {
  ...
  async update(id: string, data: Partial<ReadingSession>): Promise<ReadingSession> {
  ...
  async addEvent(sessionId: string, event: Partial<SessionEvent>): Promise<SessionEvent> {
  ...
  async findMany(params: any): Promise<ReadingSession[]> {
      const found = await this.prisma.reading_sessions.findMany({
          ...params,
          include: { contents: { select: { id: true, title: true, type: true, original_language: true } } }
      });
