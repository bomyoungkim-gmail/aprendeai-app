...
export class AdvancePhaseUseCase {
    ...
    if (toPhase === "POST") {
      if (session.phase !== "PRE" && session.phase !== "DURING") {
        throw new BadRequestException(
          "Can only advance to POST from PRE or DURING phase",
        );
      }
    }
  ...
  private async validatePostCompletion(
    ...
    const notes = await this.cornellRepository.findByContentAndUser(
      contentId,
      userId,
    );
    ...
    const hasProduction = events.some(
