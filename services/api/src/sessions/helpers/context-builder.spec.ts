...
describe('buildSessionContext', () => {
  ...
  it('should use fallbacks when data is missing', async () => {
    ...
    mockDeps.prisma.reading_sessions.findUnique = jest.fn().mockResolvedValue(null);
    mockDeps.prisma.contents.findUnique = jest.fn().mockResolvedValue(null);
    mockDeps.gamificationService.getDashboard = jest.fn().mockRejectedValue(new Error('Service down'));
    mockDeps.prisma.vocab_items.count = jest.fn().mockRejectedValue(new Error('DB error'));
    mockDeps.prisma.vocab_items.findFirst = jest.fn().mockRejectedValue(new Error('DB error'));
    mockDeps.prisma.session_events.count = jest.fn().mockRejectedValue(new Error('DB error'));
  ...
  it('should return minimal context on catastrophic failure', async () => {
    mockDeps.prisma.users.findUnique = jest.fn().mockRejectedValue(new Error('DB down'));
