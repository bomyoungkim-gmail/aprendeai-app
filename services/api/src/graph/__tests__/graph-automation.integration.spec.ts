...
describe("Graph Automation - Core Logic", () => {
  ...
  describe("Temporal Decay Formula", () => {
    it("should apply exponential decay correctly", () => {
      ...
      const decayed = decayService.calculateDecay(
        currentConfidence,
        lastReinforcedAt,
      );
    ...
    it("should decay to half after one half-life period", () => {
      ...
      const lastReinforcedAt = new Date(
        Date.now() - halfLifeDays * 24 * 60 * 60 * 1000,
      );
    ...
    it("should decay to quarter after two half-lives", () => {
      ...
      const halfLifeDays = 14;
      const lastReinforcedAt = new Date(
        Date.now() - 2 * halfLifeDays * 24 * 60 * 60 * 1000,
      );
  ...
  describe("Configuration Validation", () => {
    it("should throw error if half-life is invalid", () => {
      ...
      expect(() => new GraphDecayService(prisma)).toThrow(
        "GRAPH_DECAY_HALF_LIFE must be greater than 0",
      );
