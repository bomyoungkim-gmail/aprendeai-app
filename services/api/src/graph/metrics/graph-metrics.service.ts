...
export class GraphMetricsService {
  ...
  recordJobExecution(
    ...
    if (success) {
      this.jobSuccessCount.set(
        jobName,
        (this.jobSuccessCount.get(jobName) || 0) + 1,
      );
    } else {
      this.jobFailureCount.set(
        jobName,
        (this.jobFailureCount.get(jobName) || 0) + 1,
      );
    }
  ...
  async getPrometheusMetrics(): Promise<string> {
    ...
    for (const [jobName, durations] of this.jobDurations.entries()) {
      if (durations.length > 0) {
        ...
        metrics.push(
          `graph_job_duration_ms{job="${jobName}",quantile="0.5"} ${p50}`,
        );
        metrics.push(
          `graph_job_duration_ms{job="${jobName}",quantile="0.95"} ${p95}`,
        );
        metrics.push(
          `graph_job_duration_ms{job="${jobName}",quantile="0.99"} ${p99}`,
        );
      }
    }
    ...
    metrics.push(
      "# HELP graph_job_executions_total Total number of job executions",
    );
    metrics.push("# TYPE graph_job_executions_total counter");

    for (const [jobName, count] of this.jobSuccessCount.entries()) {
      metrics.push(
        `graph_job_executions_total{job="${jobName}",status="success"} ${count}`,
      );
    }

    for (const [jobName, count] of this.jobFailureCount.entries()) {
      metrics.push(
        `graph_job_executions_total{job="${jobName}",status="failure"} ${count}`,
      );
    }

    // Graphs processed gauge
    metrics.push(
      "# HELP graph_job_graphs_processed Number of graphs processed in last run",
    );
    ...
    try {
      ...
      metrics.push(
        "# HELP graph_total_learner_graphs Total number of learner graphs",
      );
      metrics.push("# TYPE graph_total_learner_graphs gauge");
      metrics.push(
        `graph_total_learner_graphs ${healthMetrics.totalLearnerGraphs}`,
      );

      // Average confidence
      metrics.push(
        "# HELP graph_average_confidence Average confidence score across all nodes",
      );
      metrics.push("# TYPE graph_average_confidence gauge");
      metrics.push(
        `graph_average_confidence ${healthMetrics.averageConfidence}`,
      );

      // Graphs needing comparison
      metrics.push(
        "# HELP graph_needing_comparison Number of graphs needing comparison",
      );
      metrics.push("# TYPE graph_needing_comparison gauge");
      metrics.push(
        `graph_needing_comparison ${healthMetrics.graphsNeedingComparison}`,
      );

      // Job health status (0=error, 1=warning, 2=healthy)
      metrics.push(
        "# HELP graph_job_health Job health status (0=error, 1=warning, 2=healthy)",
      );
      metrics.push("# TYPE graph_job_health gauge");

      const comparisonHealth = this.healthStatusToNumber(
        healthMetrics.lastComparisonJob.status,
      );
      const decayHealth = this.healthStatusToNumber(
        healthMetrics.lastDecayJob.status,
      );

      metrics.push(
        `graph_job_health{job="graph-comparison"} ${comparisonHealth}`,
      );
