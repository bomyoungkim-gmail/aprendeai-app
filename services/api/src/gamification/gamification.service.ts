...
export class GamificationService {
  ...
  async getDashboard(userId: string) {
    ...
    return {
      dailyActivity: dailyActivity || { minutesSpent: 0, lessonsCompleted: 0, goalMet: false },
      dailyGoal: dailyGoal || { goalType: 'MINUTES', goalValue: 15 }, // Default goal
  ...
  private async checkGoalCompletion(userId: string, activity: any) {
    ...
    const goal = await this.prisma.dailyGoal.findFirst({
      where: { userId },
      orderBy: { createdAt: 'desc' },
    }) || { goalType: DailyGoalType.MINUTES, goalValue: 15 }; // Default

    let met = false;
    if (goal.goalType === DailyGoalType.MINUTES && activity.minutesSpent >= goal.goalValue) {
      met = true;
    } else if (goal.goalType === DailyGoalType.LESSONS && activity.lessonsCompleted >= goal.goalValue) {
      met = true;
    }
  ...
  private async updateStreak(userId: string, activityDate: Date) {
    ...
    const lastMet = streak.lastGoalMetDate ? new Date(streak.lastGoalMetDate) : null;
    ...
    if (!lastMet) {
        ...
        await this.prisma.streak.update({
            where: { userId },
            data: { currentStreak: 1, bestStreak: 1, lastGoalMetDate: activityDate }
        });
    } else {
