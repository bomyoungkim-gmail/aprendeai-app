...
import { DecisionInput, DecisionSignals, SuppressReason } from '../domain/decision.types';
...
describe('DecisionService', () => {
  ...
  it('should return NO_OP when no triggers are present', async () => {
    ...
    mockPrisma.institution_policies.findFirst.mockResolvedValue(mockDefaultPolicy);
    mockPrisma.usage_events.aggregate.mockResolvedValue({ _sum: { quantity: 0 } });
  ...
  it('should force DETERMINISTIC channel when budget is exceeded', async () => {
    mockPrisma.users.findUnique.mockResolvedValue({ id: 'user_123' });
    mockPrisma.institution_policies.findFirst.mockResolvedValue(mockDefaultPolicy);
    mockPrisma.usage_events.aggregate.mockResolvedValue({ _sum: { quantity: 2000 } }); // Over budget (1000)
  ...
  describe('Suppression Scenarios (v2)', () => {
    ...
    it('T2: should suppress LLM action when Budget is exceeded', async () => {
      ...
      mockPrisma.users.findUnique.mockResolvedValue({ id: 'user_123' });
      mockPrisma.institution_policies.findFirst.mockResolvedValue(mockDefaultPolicy);
      mockPrisma.usage_events.aggregate.mockResolvedValue({ _sum: { quantity: 2000 } }); // > 1000
    ...
    it.skip('T3: should suppress LLM action during DURING phase if invisible', async () => {
      ...
      expect(mockLogRepository.logDecisionV2).toHaveBeenCalledWith(
        expect.objectContaining({
          suppressReasons: expect.arrayContaining([SuppressReason.PHASE_DURING_INVISIBLE]),
        }),
        expect.anything(),
      );
    });
