...
describe("FlowStateDetectorService", () => {
  ...
  describe("detectFlowState", () => {
    it("should return cached flow state if available", async () => {
      ...
      const result = await service.detectFlowState(
        testUserId,
        testContentId,
        testSessionId,
      );
      ...
      expect(
        mockPrismaService.reading_sessions.findUnique,
    ...
    it("should return default state on error", async () => {
      mockCacheManager.get.mockResolvedValue(null);
      mockPrismaService.reading_sessions.findUnique.mockRejectedValue(
        new Error("DB error"),
      );
  ...
  describe("gatherSignals", () => {
    it("should calculate reading velocity correctly", async () => {
      ...
      const signals = await service["gatherSignals"](
        testUserId,
        testContentId,
        testSessionId,
      );
