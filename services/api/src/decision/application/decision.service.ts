...
export class DecisionService {
  ...
  private readonly decisionCache = new Map<
    string,
    { result: DecisionOutput; timestamp: number }
  >();
  // TODO: Refactor to use CACHE_TTL.DECISION from decision.constants.ts
  // Deferred due to complexity and risk of regression in critical path
  ...
  async makeDecision(input: DecisionInput): Promise<DecisionOutput> {
    ...
    const multipliers =
      this.scaffoldingService.getThresholdMultipliers(scaffoldingLevel);
    ...
    const sessionPhase = await this.deriveSessionPhase(
      input.sessionId,
      input.contentId,
    );
    ...
    const enforcementPhase =
      sessionPhase === "PRE" || sessionPhase === "GAME"
        ? "DURING"
        : sessionPhase;
  ...
  private proposeAction(
    ...
    if (signals.chatContext?.text) {
      ...
      const intent = this.detectChatIntent(
        text,
        signals.chatContext.hasSelection,
      );
      ...
      if (intent === "SENTENCE_ANALYSIS") {
        return {
          ...
          payload: {
            ...
            hasSelection: signals.chatContext.hasSelection,
            selectedText: signals.chatContext.selectedText,
          },
        };
      }
    ...
    // SCRIPT 08: Productive Failure trigger (low mastery + content has PF assets)
      ...
      return {
        ...
        reason: "LOW_MASTERY",
        payload: {
          missionType: "PRODUCTIVE_FAILURE",
          scaffoldingAction: "SUPPORT",
        },
      };
    }
    // Post-summary check
  ...
  private async enforce(
    ...
    if (
      dcsSnapshot &&
      this.dcsHelper.shouldSuppressInvisible(phase, !!explicitAsk, w_det)
    ) {
      this.logger.debug(
        `DCS invisible mode: suppressing action (w_det=${w_det.toFixed(3)})`,
      );
      finalAction = "NO_OP";
    ...
    if (dcsSnapshot && !this.dcsHelper.isActionAllowed(finalAction, w_det)) {
      this.logger.debug(
        `DCS depth policy: action ${finalAction} not allowed (w_det=${w_det.toFixed(3)})`,
      );
      finalAction = "NO_OP";
    ...
    if (
      !explicitAsk &&
      (finalAction === "CALL_AI_SERVICE_EXTRACT" ||
        finalAction === "CALL_AGENT")
    ) {
      finalAction = "NO_OP";
    }
    ...
    if (!explicitAsk && finalAction !== "NO_OP") {
      const maxInterventions = this.scaffoldingService.getMaxInterventions(
        scaffoldingLevel as 0 | 1 | 2 | 3,
      );
    ...
    if (contentId && userId) {
      try {
        ...
        const scaffoldingState =
          (learnerProfile?.scaffolding_state_json as any) || {
        ...
        const timeSinceLastChange =
          Date.now() - new Date(scaffoldingState.lastLevelChangeAt).getTime();
      ...
      try {
        ...
        if (assessmentSignal.shouldAdjust) {
          ...
          const currentScaffoldingState =
            (learnerProfile?.scaffolding_state_json as any) || {
              currentLevel: scaffoldingLevel,
            };

          const currentLevel =
            currentScaffoldingState.currentLevel || scaffoldingLevel;
          const newLevel = this.calculateNewScaffoldingLevel(
            currentLevel,
  ...
  private detectChatIntent(
    ...
    const analysisRegex =
      /(analise|sintaxe|oração|sentence|structure|gramática|sujeito)/i;
   ...
   */
  private handleExplicitActionProposal(action: string): {
    action: DecisionAction;
    channelHint: DecisionChannel;
    reason: DecisionReason;
    payload?: any;
  ...
  private handleDoubtSpike(): DecisionOutput {
    return this.createDecision("ASK_PROMPT", "DETERMINISTIC", "DOUBT_SPIKE", {
  ...
  private isSummaryPoor(signals: DecisionInput["signals"]): boolean {
    return (
      signals.summaryQuality === "EMPTY" || signals.summaryQuality === "SHORT"
    );
  }
  ...
  async evaluateExtractionPolicy(
    ...
    const channelAfter: DecisionChannel = "LLM";
    ...
    const resultV2: DecisionResultV2 = {
      ...
      payload: {
        phase,
        caps: suppressed ? null : { maxTokens: 1000, modelTier: "flash" },
      },
      policySnapshot: {
        transferEnabled: policy.transferEnabled,
        llmBudgetDailyTokens: policy.llmBudgetDailyTokens,
    ...
    if (suppressed) {
      ...
      if (suppressReasons.includes(SuppressReason.BUDGET_EXCEEDED))
        legacyReason = "budget";
      else if (suppressReasons.includes(SuppressReason.PHASE_DURING_INVISIBLE))
        legacyReason = "phase";
   ...
   * - Otherwise → MAINTAIN
   */
  private async checkAssessmentPerformance(userId: string): Promise<{
    ...
    try {
      ...
      this.logger.error(
        `Failed to check assessment performance: ${error.message}`,
      );
      return {
        shouldAdjust: false,
        direction: "MAINTAIN",
  ...
  private calculateAvgScore(
    attempts: Array<{ score_raw: number | null }>,
  ): number {
    const validScores = attempts
      .filter((a) => a.score_raw !== null)
      .map((a) => a.score_raw!);
