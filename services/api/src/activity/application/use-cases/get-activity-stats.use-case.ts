...
export class GetActivityStatsUseCase {
  ...
  async execute(userId: string): Promise<ActivityStats> {
    ...
    const activities = await this.activityRepo.getActivities(
      userId,
      oneYearAgo,
    );
    ...
    const activeTopics = await this.activityRepo.getActiveTopicsCount(
      userId,
      sevenDaysAgo,
    );
  ...
  private calculateCurrentStreak(activities: Activity[]): number {
    ...
    const hasToday = activities.some(
      (a) => differenceInDays(today, a.date) === 0,
    );
    const hasYesterday = activities.some(
      (a) => differenceInDays(today, a.date) === 1,
    );
    ...
    for (let i = 0; i < 365; i++) {
      ...
      const hasActivity = activities.some(
        (a) => differenceInDays(checkDate, a.date) === 0,
      );
