...
export class PrismaSubscriptionRepository implements ISubscriptionRepository {
  ...
  async create(subscription: Subscription): Promise<Subscription> {
    ...
    console.log(
      `[PrismaSubscriptionRepository] Creating sub for user ${subscription.userId}. Has TX via CLS? ${!!this.txHost.tx}`,
    );
  ...
  private mapToEntity(model: any): Subscription {
    ...
    if (model.plans) {
      planEntity = new Plan({
        ...
        monthlyPrice: model.plans.monthly_price
          ? Number(model.plans.monthly_price)
          : undefined,
