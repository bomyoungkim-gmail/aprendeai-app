...
export class OutcomesService {
  ...
  private async calculateComprehension(session: any): Promise<number> {
    ...
    const quizEvents = session.events.filter(e => e.eventType === 'QUIZ_RESPONSE');
    if (quizEvents.length > 0) {
      // Assume payload has { correct: boolean }
      const correctCount = quizEvents.filter(e => e.payload?.correct === true).length;
    ...
    const checkpointEvents = session.events.filter(e => e.eventType === 'CHECKPOINT_RESPONSE');
    if (checkpointEvents.length > 0) {
      // Simple heuristic: response length indicates engagement
      const avgLength = checkpointEvents.reduce((sum, e) => 
        sum + (e.payload?.response?.length || 0), 0) / checkpointEvents.length;
      
      if (avgLength > 50) score += 10; // Good engagement
      else if (avgLength < 20) score -= 10; // Weak engagement
    }

    // Factor 3: Unknown word rate
    const unknownWordEvents = session.events.filter(e => e.eventType === 'MARK_UNKNOWN_WORD');
    ...
    if (unknownRate > 2) score -= 15; // High unknown rate = lower comprehension
  ...
  private async calculateProduction(session: any): Promise<number> {
    ...
    const keyIdeaEvents = session.events.filter(e => e.eventType === 'MARK_KEY_IDEA');
    ...
    const productionEvents = session.events.filter(e => e.eventType === 'PRODUCTION_SUBMIT');
    if (productionEvents.length > 0) {
      const avgLength = productionEvents.reduce((sum, e) => 
        sum + (e.payload?.text?.length || 0), 0) / productionEvents.length;
      
      if (avgLength > 200) score += 15; // Quality synthesis
  ...
  private async calculateFrustration(session: any): Promise<number> {
    ...
    const checkpointEvents = session.events.filter(e => e.eventType === 'CHECKPOINT_RESPONSE');
    const weakResponses = checkpointEvents.filter(e => 
      (e.payload?.response?.length || 0) < 10
    ).length;
    ...
    const quizEvents = session.events.filter(e => e.eventType === 'QUIZ_RESPONSE');
    const incorrectCount = quizEvents.filter(e => e.payload?.correct === false).length;
