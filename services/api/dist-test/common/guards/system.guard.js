...
let SystemGuard = class SystemGuard {
    ...
    canActivate(context) {
        ...
        if (!user || !user.systemRole) {
            throw new common_1.ForbiddenException("Access denied: System Role required");
        }
        if (!roles.includes(user.systemRole)) {
            throw new common_1.ForbiddenException("Access denied: Insufficient System privileges");
        }
        return true;
    }
};
