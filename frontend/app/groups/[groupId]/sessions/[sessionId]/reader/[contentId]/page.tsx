...
export default function GroupReaderPage({ params }: GroupReaderPageProps) {
  ...
  const handleCreateAnnotation = useCallback(
    ...
    [createAnnotation, params.contentId, showToast, clearSelection]
  );
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mb-2">Loading Group Study Session...</h2>
          <p className="text-gray-500">Preparing collaborative reader</p>
          {isInSession && (
            <p className="text-sm text-blue-600 mt-2">Group: {groupId}</p>
          )}
        </div>
      </div>
    );
  ...
  return (
    <>
      <div className="relative h-screen flex">
          ...
          <AnnotationToolbar
            ...
            onCreateAnnotation={(type, color, text) => {
              handleCreateAnnotation({
                ...
                color,
                text,
                visibility: 'GROUP', // Group annotations are visible to all members
              });
            }}
        ...
        {/* Annotations Sidebar - Now with REAL groupId! */}
        {showAnnotations && (
          <div className="fixed right-0 top-0 h-full w-80 border-l border-gray-200 bg-white shadow-lg z-10">
            <AnnotationsSidebar
              contentId={params.contentId}
              groupId={groupId} // ✅ Dynamic from context!
            />
            {isInSession && (
              <div className="absolute bottom-0 left-0 right-0 p-2 bg-blue-50 border-t text-xs text-center text-blue-700">
                📚 Group Study Mode
              </div>
            )}
          </div>
